...
mkdir -p out/logs
LOG_PATH="out/logs/acceptance_nc_201.log"
TEST_PATH="app/tests_py/acceptance/test_atnc_201_provider_feature_flags.py"

if [[ ! -f "$TEST_PATH" ]]; then
  echo "ERROR: missing required next-cycle acceptance test: $TEST_PATH" >&2
  echo "Implement AT-NC-201 suite before running this gate." >&2
  exit 2
fi
...
if [[ -x worker/.venv/bin/python ]]; then
  ...
  echo "ERROR: python runtime unavailable for AT-NC-201" >&2
  exit 1
fi
