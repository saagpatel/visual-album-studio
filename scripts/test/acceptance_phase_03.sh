...
if rg -n "SCRIPT ERROR|Parse Error|Failed to load script|\[FAIL\]" "$PRODUCT_LOG" >/dev/null 2>&1; then
  echo "ERROR: product-path AT-003 log contains script/runtime failures. See $PRODUCT_LOG" >&2
  exit 1
fi

if [[ -x worker/.venv/bin/python ]]; then
  set +e
  PYTHONPATH=worker:app/src/core_py worker/.venv/bin/python -m pytest -q app/tests_py/acceptance/test_at003_phase3.py | tee out/logs/acceptance_phase_03_harness.log
  HARNESS_EXIT=$?
  set -e
  if [[ $HARNESS_EXIT -ne 0 ]]; then
    echo "WARN: Harness AT-003 regression failed (non-gating during rebaseline)." >&2
  fi
fi
