...
if command -v godot >/dev/null 2>&1; then
  VAS_REPO_ROOT="$ROOT_DIR" godot --headless --path app --script res://tests/phase5_acceptance.gd 2>&1 | tee "$PRODUCT_LOG"
elif command -v godot4 >/dev/null 2>&1; then
  VAS_REPO_ROOT="$ROOT_DIR" godot4 --headless --path app --script res://tests/phase5_acceptance.gd 2>&1 | tee "$PRODUCT_LOG"
else
