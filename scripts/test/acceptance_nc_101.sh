...
mkdir -p out/logs
LOG_PATH="out/logs/acceptance_nc_101.log"
TEST_PATH="app/tests_py/acceptance/test_atnc_101_preset_exchange_trust_ui.py"

if [[ ! -f "$TEST_PATH" ]]; then
  echo "ERROR: missing required next-cycle acceptance test: $TEST_PATH" >&2
  echo "Implement AT-NC-101 suite before running this gate." >&2
  exit 2
fi
...
if [[ -x worker/.venv/bin/python ]]; then
  ...
  echo "ERROR: python runtime unavailable for AT-NC-101" >&2
  exit 1
fi
