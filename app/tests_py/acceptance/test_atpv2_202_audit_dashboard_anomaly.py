...
def test_atpv2_202_audit_dashboard_anomaly(runtime):
    ...
    runtime.db.execute(
        ...
        ("diag_at1", "x", "error", "{}", now),
    )
    runtime.db.execute(
        "INSERT INTO connector_diagnostics(id, connector, severity, payload_json, created_at) VALUES (?, ?, ?, ?, ?)",
        ("diag_at2", "x", "error", "{}", now),
    )
    runtime.db.execute(
        "INSERT INTO connector_diagnostics(id, connector, severity, payload_json, created_at) VALUES (?, ?, ?, ?, ?)",
        ("diag_at3", "x", "error", "{}", now),
    )
    runtime.db.execute(
        "INSERT INTO connector_diagnostics(id, connector, severity, payload_json, created_at) VALUES (?, ?, ?, ?, ?)",
        ("diag_at4", "instagram", "warn", "{}", now),
    )
    runtime.db.execute(
        "INSERT INTO cloud_sync_queue(id, project_id, envelope_json, status, error_code, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ("q_at1", "project_atpv2_202", "{}", "failed", "E_FAIL", now, now),
    )
    runtime.db.execute(
        "INSERT INTO cloud_sync_replay_log(id, project_id, queue_id, sequence, status, detail_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ("r_at1", "project_atpv2_202", "q_at1", 1, "failed", "{}", now),
    )
    runtime.db.execute(
        "INSERT INTO cloud_sync_replay_log(id, project_id, queue_id, sequence, status, detail_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ("r_at2", "project_atpv2_202", "q_at1", 2, "failed", "{}", now),
    )
    runtime.db.execute(
        ...
        ("c_at1", "project_atpv2_202", "clip_1", "user_1", 2, "{}", now),
    )
    runtime.db.execute(
        "INSERT INTO collaboration_conflicts(id, project_id, resource_id, winner_actor_id, winner_sequence, resolution_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ("c_at2", "project_atpv2_202", "clip_2", "user_2", 3, "{}", now),
    )
    runtime.db.commit()

    service = AuditDashboardServiceV1(runtime.db)
    dashboard = service.dashboard(project_id="project_atpv2_202", since_epoch=now - 1)
