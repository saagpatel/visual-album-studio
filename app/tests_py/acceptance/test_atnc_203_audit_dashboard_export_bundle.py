...
def test_atnc_203_audit_dashboard_export_bundle(runtime, test_root):
    ...
    for idx in range(3):
        runtime.db.execute(
            ...
            (f"diag_atnc203_{idx}", "youtube", "error", '{"refresh_token":"secret"}', now),
        )
    runtime.db.commit()

    service = AuditDashboardExportServiceV1(runtime.db)
    out_dir = Path(test_root) / "out" / "audit_exports"
    result = service.export_bundle(project_id="proj_atnc203", output_dir=out_dir)
    ...
    listed = service.list_exports(project_id="proj_atnc203")
